...
public class UserController {
    ...
    @PatchMapping(value = "/update-users", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<CreateUser> updateUser(@RequestHeader("userId") Long userId,
                                                 @RequestBody CreateUser updateUser,
                                                 @RequestPart(value = "avatarImg", required = false) MultipartFile image)     {
        CreateUser createUser = userService.updateUser(userId, updateUser, image     );
